...
for page in range(1, 4):
    ...
    for card in cards:
        try:
            ...
            # ✅ 제목 필터: '프론트엔드' or 'frontend'
            title_lower = title.lower()
            if '프론트엔드' in title_lower or 'frontend' in title_lower:
...
# ✅ CSV 저장
with open("jumpit_frontend_all.csv", "w", newline="", encoding="utf-8-sig") as f:
    writer = csv.writer(f)
    writer.writerow(["회사명", "공고", "제목", "링크", "기술 스택", "주요업무", "자격요건"])
    writer.writerows(job_list)

print("📄 프론트엔드(한글+영어) CSV 저장 완료: jumpit_frontend_all.csv")
